...
async fn sync_single_file(
    ...
    match comparison.state {
        ...
        SyncState::NeedsUpload => {
            ...
            if file_size > crate::storage::MAX_FILE_SIZE {
                log::error!(
                    "File too large: {} ({} bytes, max {} bytes)",
                    comparison.path,
                    file_size,
                    crate::storage::MAX_FILE_SIZE
                );
                return Err(CommandError::Storage(StorageError::FileTooLarge(
                    file_size,
                    crate::storage::MAX_FILE_SIZE,
                )));
            }
            ...
            if file_size >= crate::storage::PROGRESS_THRESHOLD {
                log::info!(
                    "Uploading large file: {} ({:.2} MB)",
                    comparison.path,
        ...
        SyncState::NeedsDownload => {
            ...
            // Log progress for large files (>= 100MB) based on remote size
            if let Some(remote_size) = comparison.remote_size {
                if remote_size >= crate::storage::PROGRESS_THRESHOLD {
                    log::info!(
                        "Downloading large file: {} ({:.2} MB)",
                        comparison.path,
            ...
            // Check file size after download (5GB limit)
            let file_size = data.len() as u64;
            if file_size > crate::storage::MAX_FILE_SIZE {
                log::error!(
        ...
        SyncState::Conflict => {
            ...
            // Check local file size (5GB limit)
            let local_size = local_data.len() as u64;
            if local_size > crate::storage::MAX_FILE_SIZE {
                log::error!(
                    "Local file too large: {} ({} bytes, max {} bytes)",
                    comparison.path,
                    local_size,
                    crate::storage::MAX_FILE_SIZE
                );
            ...
            // Check remote file size (5GB limit)
            let remote_size = remote_data.len() as u64;
            if remote_size > crate::storage::MAX_FILE_SIZE {
                log::error!(
                    "Remote file too large: {} ({} bytes, max {} bytes)",
                    comparison.path,
                    remote_size,
                    crate::storage::MAX_FILE_SIZE
                );
            ...
            let conflicted_metadata = FileMetadata::new(local_size, chrono::Utc::now(), None, None);
